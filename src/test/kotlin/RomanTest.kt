...
class RomanTest {
    ...
    @Nested
    @DisplayName("Roman Base: k=IV -> M")
    inner class TestRomanBase {
    ...
    @Nested
    @DisplayName("Roman Tricky: k=CMXCIX, MMMDCCCLXXXVIII, MMMCMXCIX")
    inner class TestRomanTricky {

        @TestFactory
        fun romanTests() = listOf(
            "CMXCIX" to "CMXCIX",
            "MMMDCCCLXXXVIII" to "MMMDCCCLXXXVIII",
            "MMMCMXCIX" to "MMMCMXCIX",
        ).map { (input, input2) ->
            DynamicTest.dynamicTest(input) {
                assertEquals(romanToIntAns(input), testMethod.invoke(testObject, input2))
            }
